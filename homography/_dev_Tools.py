...
def hologram_color(image, scale = 260):
    ...
    for channel_id, c in zip(channel_ids, colors):
        histogram, bin_edges = np.histogram(
            image[:, :, channel_id], bins=scale, range=(0, scale)
        )
...
def funcBrightContrast(bright=0):
    bright = cv.getTrackbarPos('bright', 'brightnessContrasr')
    contrast = cv.getTrackbarPos('contrast', 'brightnessContrasr')
...
def change_brightness_contrast(img_path):

    original = cv.imread(img_path, 1)
    global img
    img = original.copy()

    cv.namedWindow('brightnessContrasr',1)
    ...
    cv.createTrackbar('bright', 'brightnessContrasr', bright, 2*255, funcBrightContrast)
    cv.createTrackbar('contrast', 'brightnessContrasr', contrast, 2*127, funcBrightContrast)
    funcBrightContrast(0)
    cv.imshow('brightnessContrasr', original)
