...
def brightness(x):
    cap.set(cv.CAP_PROP_BRIGHTNESS, x-100)
...
def AUTO_EXPOSURE(x):
    cap.set(cv.CAP_PROP_AUTO_EXPOSURE, x)
...
cv.namedWindow('Camera_Settings', cv.WINDOW_NORMAL)
cv.resizeWindow("Camera_Settings", 700, 250)

cv.createTrackbar('SATURATION','Camera_Settings',100,200,SATURATION)
cv.createTrackbar('HUE','Camera_Settings',0,179,hue)
cv.createTrackbar('BRIGHTNESS','Camera_Settings',100,200,brightness)
...
# New 
# cv.createTrackbar('CAP_PROP_AUTO_EXPOSURE','Camera_Settings',0,3,AUTO_EXPOSURE)
